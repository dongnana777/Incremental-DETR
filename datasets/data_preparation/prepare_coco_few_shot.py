...
def parse_args():
    ...
    parser.add_argument("--seeds", type=int, nargs="+", default=[0, 10], help="Range of seeds")
...
def generate_seeds(args):
    ...
    for i in range(args.seeds[0], args.seeds[1]):
        ...
        sample_shots_list = []
        sample_imgs_list = []
        for shots in [5]: # few-shot
            for c in ID2CLASS_novel.keys():
                img_ids = {}
                for a in anno[c]:
...
def get_save_path_seeds(path, shots, seed):
    prefix = "full_box_{}shot_novel_train".format(shots)
    save_dir = os.path.join('./coco2017/annotations', "seed" + str(seed))
...
if __name__ == "__main__":
    ...
    CLASS2ID = {v: k for k, v in ID2CLASS_novel.items()}
